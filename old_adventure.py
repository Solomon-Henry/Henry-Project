...
Stop_A_Text_List = [Game_Title,"CONTINUE",Game_Description,"SETTER00","SETTER11",Game_Instructions]
Game_Credits_0 = " for playing My Project. \nIt took a while to do \nso hopefully I'll get a good grade!\n:)"
Game_Copyright = "\n© 2022 Solomon Henry Corporations"
Stop_F_Text_List = [Game_Credits_0,Game_Copyright]
Stop_0:str = "Courtroom"
Stop_0_Text_0:str = "',the judge says"
...
Stop_8_Text_0:str = "!' booms a computerized voice"
...
User_Script_Prompt_List = [
     [["'It's"], ["Big Words for a buncha of Dictators"], ["There is always a price to freedom"],["*You then proceed to dislodge your tooth a metal railing then you shot tooth into the judge's eye*"],["Salvo is a free world! No matter whose flag you shove in her soil! "]],
     [[""], [""], [""]],
     ...
     [["'Takes a lot more than that to kill the Me!'"], [""], [""]],
     [[], [], []]
]
...
User_Commands = ["YES","NO","1","0","NORTH","SOUTH","WEST","EAST","HELP","QUIT","EXAMINE"]
Name:str = ""
Easy_Mode_Status:bool = False
User_Customization_Input_List = [Name,Easy_Mode_Status] 
Customization_Strings = ["\nWhat will your name will be? ", "\nTurn on Easy Mode?\nThis will allow you to always choose the correct option simply by pressing Enter "]
Location_Details = [[["\nYou're in a courtroom"],["\nYou're in the outlands, but where exactly, you don't know"]],
[["\nYou are now in a flying Dropship"],["\nYou're somewhere above the flying city of Olympus"]],
[["\nYou're at Hammond Labs with an angry enemy squad with guns"],["\nNorth of you is Turbine, the main turbine that keeps the city afloat"],["\nSouth of you is Solar Array, its name speaks for itself"],["\nEast of you is Grow Towers, a place where large structures are covered with nature"],["\nWest of you is the Estates, fancy houses for fancy people"]],
[["\nYou are at the Estates, surrounded by many enemies"],["\nNorth of you is Oasis, fancy place with a pool, maybe you should visit sometime"],["\nSouth of you is Hydroponics, Olympus' water system"],["\nEast of you is Hammond Labs, You were literally just there"],["\nWest of you is a Secret Trapdoor, Seems interesting."]],
[["\nYou're in a Secret Trapdoor, maybe you should get out somehow"],["\nLocation is unknown, somehwere beneath Olympus"]],
[["\nYou're now in Phase Driver, a nice place if you ignore the 2 squads killing each other in front of you"],["\nNorth of you is Terminal, a structure that connects 2 convenient portals"],["\nSouth of you is a fall of more than 1000 meters, if you're feeling like a quitter"],["\nEast of you is Bonsai Palace, a palace where the Apex champions will be shown off"],["\nWest of you is again Hydroponics"]],
[["\nYou're now in Solar Array, where there are many solar panels around"],["\nNorth of you is Hammond Labs, where you first landed"],["\nSouth of you is Icarus, a ship with the deadlist virus in the world,\nsafe to say that you shouldn't go there"],["\nEast of you is the Orbital Cannon, its name speaks for itself"],["\nWest of you is Bonsai Palace, a palace where the Apex champions will be shown off"]],
[["\nYou're near Bonsai Palace, but a fight is approaching from the horizon"],["\nNorth of you is Solar Array, you were just there"],["\nSouth of you is Bonsai Palace, its buildings look so nice"],["\nEast of you is Icarus, the virus ridden ship"],["\nWest of you is Phase Driver, you were there a while ago"]],
[["\nYou're at the Champion's Podium, Yay, you're an Apex Champion."],["\nWho cares where you are, you're on top of the world as an Apex Champion"]],
[["\nYou're now going back to jail in a Prison Dropship, you didn't forget you were a criminal right?"],["\nYou're in Jail, that is all you need to know"]]]
Confirmation_Message_Name:str = " shall be your name"
Confirmation_Message_Easy_Mode:str = "Easy mode is enabled"
Error_Message:str = "\nYou have entered an invalid command.\nPlease enter a valid one.\nYou may enter 'Help' to access all of the valid commands"
Help_Message:str = "\nValid Commands:\n Yes or No to contine. (May also be substituted with 1 or 0\nNorth,South,West,East to look at and gather information on nearby locations\nQuit to stop the game after finishing exploring the location.\nExamine to find information on current location.\nHelp to see tips again"
Continue_Message:str = "Type 'Yes' to Continue"
Ready_Up_Prompt:str = "Return to game?"
Quit_Status:bool = False
Quit_Status_Message:str = "Game will be able to be exited upon completing the Story at this location."
...
def Setter(v):
    global Name
    global Confirmation_Message_Name
    global Game_Credits_0
    global Stop_0_Text_0
    global Stop_8_Text_0

    if v == "00":
        Name = input(f"{Customization_Strings[0]}")
        Confirmation_Message_Name = Name + Confirmation_Message_Name
        Stop_F_Text_List[0] = "\nThank you, player" + Name + Game_Credits_0
        Stop_0_Text_List[0] = "\n'" + Name + Stop_0_Text_0
        Stop_8_Text_List[0] = "\n*You and your team are soon transported to the Champion's Podium\nwhere the press takes so many pictures of the newest Apex Legends*\n'THE NEWEST APEX LEGENDS TODAY ARE OCTANE, LIFELINE, & '" + Name + Stop_8_Text_0
        User_Script_Prompt_List[0][0] = "The name is " + Name
        print(f"{Confirmation_Message_Name}")
    elif v == "11": 
        reponse = input(f"{Customization_Strings[1]}")
        if reponse == "" or reponse.upper == "Yes":
            print(f"{Confirmation_Message_Easy_Mode}")
            Easy_Mode()
        

# set up way to activate easy mode
def Easy_Mode():
    global Easy_Mode_Status
    global Continue_Message
    Continue_Message = "Press Enter to Continue"
    Easy_Mode_Status = True
    User_Commands.append("")
# To get the corresponding index of the users response
def Response_Finder(v):
    ans = []
    for i in range(len(User_Commands)):
        if User_Commands[i] == v.upper():
            ans.append(i)
            break
    if len(ans) != 0:  
        return ans
    elif Easy_Mode_Status == True:
        ans.append(11)
        return ans
    else: 
        ans.append(-1)
        return ans
def Command_Checker(p):
    global Quit_Status
    if Response_Finder(p) == -1 and Easy_Mode_Status == False:
        Asterisk_separator(0,0)
        print(Error_Message)
        repeat = input(Continue_Message)
        Asterisk_separator(0,0)
        Command_Checker(repeat)
    elif Response_Finder(p)[0] >= 4 and Response_Finder(p)[0] <= 7:
        Asterisk_separator(0,0)
        Examine(Response_Finder(p)[0])
        Asterisk_separator(0,0)
        repeat = input(Continue_Message)
        Asterisk_separator(0,0)
        Command_Checker(repeat)
    elif (Response_Finder(p)[0] >= 0 and Response_Finder(p)[0] <= 3) or (Response_Finder(p)[0] == 11 and Easy_Mode_Status) :
        Asterisk_separator(0,0)
        Path_Checker(Response_Finder(p))
        Asterisk_separator(0,0)
    elif Response_Finder(p)[0] == 8:
        Asterisk_separator(0,0)
        print(Help_Message)
        repeat = input(Continue_Message)
        Asterisk_separator(0,0)
        Command_Checker(repeat)
    elif Response_Finder(p)[0] == 9:
        Quit_Status = True
        Asterisk_separator(0,0)
        print(Quit_Status_Message)
        input(Continue_Message)
        Asterisk_separator(0,0)
    elif Response_Finder(p)[0] == 10:
        Asterisk_separator(0,0)
        Examine()
        repeat = input(Continue_Message)
        Asterisk_separator(0,0)
        Command_Checker(repeat)
#To find info on current location
def Examine(a=False):
    if not a:
        print(f"Current stop is {Location_List[Stop_Marker]}")
        print(Location_Details[Find_Stop_Marker()][0])
    elif len(Location_Details[Find_Stop_Marker()])<3:
        print(f"Current stop is {Location_List[Stop_Marker]}")
        print(Location_Details[Find_Stop_Marker()][1])
    else:
        loc:int
        if int(a) == 7:
            loc = int(a)-4
            ...
            loc = a-3
        print(f"Current stop is {Location_List[Stop_Marker]}")
        print(Location_Details[Find_Stop_Marker()][loc])

# To see if user chose the correct response and can go on alive
def Path_Checker(res):
    if res or not res:
        print("Good choice, you have decided to ")
# To show graphical locations nearby
def Cardinal_Directions(Dir):
    Dir = 0
...
def Script_Runner(n):
    ...
    for i in Text_List_List[n]:
        ...
        elif Text_List_List[n][cnt][0:-2] == "SETTER":
            Setter(Text_List_List[n][cnt][-2:len(Text_List_List[n][cnt])])
        elif Text_List_List[n][cnt] == "CONTINUE":
            Asterisk_separator(0,0)
            Continue_Prompt()
            Asterisk_separator(0,0)
        else: 
...
def Restart():
    ...
    global User_Stop_Cont
    global User_Script_Cont
    ans = input("You can choose to restart by typing 'y' or quit by pressing enter: ")
    if ans == "y":
        GoTo(0)
        Game_Loop_Count = 0
        Game_Loop()
